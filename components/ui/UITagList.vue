<template>
	<ul v-if="tags?.length" class="UITagList gap-3 flex flex-wrap">
		<li v-for="tag of tags" :key="tag.node.id" class="text-sm font-semibold">
			<NuxtLink
				v-if="tag.linkUrl"
				class="flex items-center gap-1 pl-2 pr-3 py-1.5 UILink rounded-full shadow-border bg-frost-200"
				:href="tag.linkUrl"
				external
				target="_blank">
				<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 256 256">
					<path
						fill="currentColor"
						d="M136.37 187.53a12 12 0 0 1 0 17l-5.94 5.94a60 60 0 0 1-84.88-84.88l24.12-24.11A60 60 0 0 1 152 99a12 12 0 1 1-16 18a36 36 0 0 0-49.37 1.47l-24.1 24.08a36 36 0 0 0 50.92 50.92l5.94-5.94a12 12 0 0 1 16.98 0m74.08-142a60.09 60.09 0 0 0-84.88 0l-5.94 5.94a12 12 0 0 0 17 17l5.94-5.94a36 36 0 0 1 50.92 50.92l-24.11 24.12A36 36 0 0 1 120 139a12 12 0 1 0-16 18a60 60 0 0 0 82.3-2.43l24.12-24.11a60.09 60.09 0 0 0 .03-84.91Z" />
				</svg>

				{{ tag.node.name }}
			</NuxtLink>

			<span v-else class="block px-3 py-1.5 rounded-full shadow-border text-gray-500 bg-frost-200">
				{{ tag.node.name }}</span
			>
		</li>
	</ul>
</template>

<script setup lang="ts">
	const props = defineProps<{
		tags: { node: { id: string; name?: string }; linkUrl?: string }[] | undefined
		linkUrl?: string
	}>()

	const tags = computed(() => {
		const allTags = [...(props.tags || [])]
		if (props.linkUrl) {
			// get base domain name without everything else
			let domain = props.linkUrl

			try {
				const url = new URL(props.linkUrl)
				domain = url.hostname.replace('www.', '')
			} catch (error) {}

			allTags.unshift({
				node: {
					id: 'link',
					name: domain,
				},
				linkUrl: props.linkUrl,
			})
		}
		return allTags
	})
</script>
